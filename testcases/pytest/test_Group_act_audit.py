...
审核:test_Auditbussio   --->这里的操作会存在：审核不通过后数据移到最后一条数据所以要额外写断言，找到位置并进行核对是否状态成功。【暂未写】
不存在查询：test_search_shopinfo_not_exist
存在的查询：test_search_shopinfo_exist
'''


class TestAuditgroupact(object):
    ...
    def test_goto_aduit_groupact(self):
        """
        进入小组活动审核页
        :return:
        """
    ...
    # 取消删除小组活动审核数据
    def test_del_cancel_groupact(self):
        """
        这是商家认证信息删除取消操作用例
        :return:
        """
        ...
        assert total == cancel_total
        sleep(1)
        # self.login.driver.quit()

    # 删除小组活动审核数据
    def NO_test_del_succ_groupact(self):
        """
        这是商家认证信息删除操作用例--删除的是列表中第一条数据
        :return:
        """
        # 获取列表数
        total = util.get_total(self.login.driver, '.ant-pagination-total-text')
        ...
        assert total - 1 == del_total

        sleep(1)

    def test_Auditgroupact(self):
        '''
        这里是小组活动审核
        :return:
        '''
        # 获取列表总数，因为执行审核不通过的话，数据会在最后一行，但是前面执行了删除，所以数据是动态的，但是可以根据xpath来构造；数据有多少条就在多少
        total = util.get_total(self.login.driver, '.ant-pagination-total-text')
        target_column = int(total)
        print('得到的数为：', target_column)
        # 重新写入xpath,将传入的数传进去，一会定位就按照这个穿【审核未通过的到最后一条，但只限于少于10条数据】
        xpath = f"/html/body/app-root/div/app-default/app-def-layout-content/nz-layout/nz-layout/nz-layout/nz-content/div/div/app-dept/div/app-card-table-wrap/nz-card/div[2]/app-tree-table/nz-table/nz-spin/div/div/nz-table-inner-scroll/div/table/tbody/tr[{target_column}]/td[4]"
        ...
        if status.text == '审核通过':
            ...
            sleep(2)
            # 判断修改后的审核状态
            changes = self.login.driver.find_element(By.XPATH, xpath)
            # 断言查看是否审核成功
    ...
    def test_search_groupact_not_exist(self):
        '''
        这里是信息审核列表--不存在
        :return:
        '''
    ...
    def test_search_groupact_exist(self):
        '''
        这里是查询信息审核列表--存在
        :return:
        '''
...
if __name__ == '__main__':
    pytest.main('-vs', '[test_Group_act_audit.py]', '--reruns=2')
