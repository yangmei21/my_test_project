...
'''
驴友日记发布审核:
取消删除:test_del_cancel_travel_diary
删除:test_del_succ_travel_diary
审核:test_Audit_travel_diary
--->这里的操作会存在：审核不通过后数据移到最后一条数据所以要额外写断言，找到位置并进行核对是否状态成功。
数据过多我会把数据先进行状态筛选，然后分别操作o(*￣▽￣*)ブ

不存在查询：test_search_travel_diary_not_exist
存在的查询：test_search_travel_diary_exist
'''


class Test_Audit_travel_diary(object):
    ...
    # 取消驴友日记发布审核数据
    def ntest_del_cancel_travel_diary(self):
        """
        这是驴友日记删除取消操作用例
        :return:
        """
    ...
    # 删除驴友日记审核数据
    def ntest_del_suc_travel_diary(self):
        """
        这是驴友日记删除操作用例--删除的是列表中第一条数据
        :return:
        """
        # 获取列表数
        total = util.get_total(self.login.driver, '.ant-pagination-total-text')
    ...
    def ntest_Audit_travel_diary(self):
        '''
        这里是驴友日记审核
        :return:
        '''
    ...
    def test_search_travel_diary_not_exist(self):
        '''
        这里是查询驴友日记审核列表--不存在
        :return:
        '''
    ...
    def test_search_travel_diary_exist(self):
        '''
        这里是查询驴友日记信息审核列表--存在
        :return:
        '''
...
if __name__ == '__main__':
    pytest.main('-vs', '[test_Trave_diary_audit.py]', '--reruns=2')
